...
\textbf{\textit{Expert for CodeMentor.io}}
	...
	\item Worked in an alige environment, helped plan sprints and created user stories, and participated in daily scrum sessions. 
	  ...
	  %\begin{itemize} 
	    %\setlength\itemsep{0em}
	     %\item Received data from Amazon AWS in the form of a byte array
	    %\item Parsed that data into a Google Protocol Buffer 
	    %\item Stored that data into a custom data structure that allowed for easy manipulation of the Google Protocol Buffer attributes 
	    %\item Send that data back to Amazon AWS
	  %\item \textit{Application's Goal}:
that when given a Google Protocol Buffer randomly added attributes to the Protocol Buffer 
 and populated them with values, so that the new Protocol Buffer could test the robustness of various applications to non-breaking changes.%\end{itemize}
	\item The application pulled its data from an AWS S3 bucket and send data back into the bucket
	\end{itemize}  
	%\textbf{\textit{Android Application}}
	%\hfill {\bf 07/2012 - Present}
	%\begin{itemize}
	  %\item Designed Android Applications for Greenville Public School District 
	%\end{itemize}
  ...
  \setlength\itemsep{0em}
	...
	\begin{itemize}
	  %\item Graded papers and provides one-on-one tutoring for students 
  ...
  \setlength\itemsep{0em}
	  ...
	  \item Taught Computer Science concepts to middle school and high school students %via Lego Mindstorm Kits, Scratch, HTML, CSS, Java, and Python 
	  \item Mainly taught students who are underrepresented in Computer Science % or students who are from areas where schools have a difficulty finding Computer Science instructors
...
%		Arudino & C                    & C++                    & C\#  \\
%		Clojure  & Mathematica    & Java                    & Python \\ 
%		R         & Racket             & Latex                   & HTML5 \\
%		CSS3    & PHP                 & ASP.NET MVC       & JavaScript  \\ 
%		jQuery  & BootStrap(CSS) & Foundation(CSS)   & Android \\
%		Git       & Clojure             & Scala                   & Protocol Buffers  \\
%		MySQL & & & \\
%	\end{tabular}
...
%\textbf{Databases} \\
	%\begin{tabular}{| l | l | l | l |}  
		%\hline
	%	SQL Server & Trans-Act SQL & MySQL & SSRS Reports  \\
		%\hline
	%\end{tabular}
...
%\section{Awards}
%\begin{itemize}
 % \item IIT 1\textsuperscript{st} and 6\textsuperscript{th} Semester Dean's
  %List
%\end{itemize}
\end{resume}
